...
def indirect_search(ontoexpline, operation, parameters):
        ...
        for item in search_program_op:
            if operation in item.is_a:
                ...
                search_attribute_value_on_db.execute(
                        f'select id, {attribute} from {table} where modelgeneratormodule_raxml_task_id = \'{search_task_id_in_db[0][0]}\' and {attribute} = \'{port_value}\';')
                ...
                # for dataflows in search_data_transformation_by_expLine(item.hasId):
                #     search_dt_by_dataflow = connection.cursor()
                #     search_dt_by_dataflow.execute(
                #         f'select data_transformation_id from data_transformation_execution where dataflow_execution_id_2023 = \'{dataflows[0]}\';')
                #     search_dt_by_dataflow = search_dt_by_dataflow.fetchall()
                #     print("** dataflow: ", dataflows[0], " atividades : ", search_dt_by_dataflow)
